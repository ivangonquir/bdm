...
# FIX: specify the .env path explicitly so it works from any working directory
...
for year in range(START_YEAR, END_YEAR + 1):
    ...
    # FIX: datatypeid must be sent as separate params, not comma-joined in one string.
    # The NOAA API expects repeated query params: ?datatypeid=TMIN&datatypeid=TMAX&...
    params = [
    ...
    if response.status_code == 200:
        ...
        print(f"[NOAA] API Error {response.status_code} for {year}")
        raise Exception(f"NOAA API error {response.status_code}")  # FIX: fail loudly
